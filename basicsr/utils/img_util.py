...
def tensor2img(tensor, rgb2bgr=True, out_type=np.uint8, min_max=(0, 1)):
    ...
    for _tensor in tensor:
        ...
        # Replace NaN/Inf before clamping: clamp_ does NOT handle NaN (undefined behavior in PyTorch).
        # NaN values typically come from AMP (fp16 overflow) or unstable model outputs.
        _tensor = torch.nan_to_num(_tensor, nan=0.0, posinf=min_max[1], neginf=min_max[0])
        ...
        if out_type == np.uint8:
            ...
            # Extra np.clip ensures no residual NaN/Inf survives color space conversion (e.g. cvtColor).
            img_np = np.clip(img_np * 255.0, 0, 255).round()
